...
export function OrgProducts() {
  ...
  const switchHandler = (item: IResponseProductOrg) => {
    const updatedItem = {
      ...item,
      is_momo_ecommerce_enable: !item.is_momo_ecommerce_enable,
    }
  ...
  const getTotalPage = (data: ResponseType<IResponseProductOrg[]>) => {
                      ...
                      {productList.map((item, index: number) => (
                        <tr key={item.id} className='text-gray-400 fw-bold fs-7 gs-0'>
                          ...
                          <th className='sorting text-gray-800'>
                            {formatPrice(
                              formatSalePriceProduct(Number(item.special_price), Number(item.special_price_momo))
                            )}
                          </th>
