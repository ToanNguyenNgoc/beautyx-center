...
import { AppSnack, PermissionLayout, SelectionOrg } from 'app/components';
//@ts-ignore
...
const RenderElement: FC<RenderElementProps> = ({ formik, origin, setOrigin, banner }) => {
  ...
  return (
    <div className='mt-8'>
      ...
      {
        <div className="col" style={{ display: type === BANNER_TYPE.HTML ? 'block' : 'none' }}>
          <label className="filter-row_label">Nhập content</label>
          <FroalaEditor
            model={formik.values.htmlTemplate}
            onModelChange={(e: string) => formik.setFieldValue('htmlTemplate', e)}
            tag='textarea'
          />
        </div>
      }
